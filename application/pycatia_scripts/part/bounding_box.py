from pycatia.enumeration.enumeration_types import cat_constraint_type, cat_constraint_mode, cat_vis_property_show
...
def create_bounding_box():
    ...
    application = pt_part_document.application
    ...
    if mb == 2:
        output['errors'].append('Process has been cancelled.')
    ...
    sel = selection.select_element2(filter_, 'Select bounding box AxisSystem.', False)

    if sel == 'Cancel':
        output['errors'].append('Selection process cancelled.')
    ...
    sel = selection.select_element2(filter_, 'Select a face of the body to be bounded.', False)
    ...
    sketch_bbox_1.name = 'Sketch.BoundingBox'
    ...
    parameter_bbox_z_min = parameters.create_dimension('BBoxOffset_ZMin', 'Length', 10)
    parameter_bbox_z_max = parameters.create_dimension('BBoxOffset_ZMax', 'Length', 10)
    ...
    # link zmin / zmax to line height.
    relations.create_formula('Formula.Zmin', '', line_guide_start, parameter_bbox_z_min.name.split('\\')[1])
    relations.create_formula('Formula.Zmax', '', line_guide_end, parameter_bbox_z_max.name.split('\\')[1])
    ...
    x = round(spa_workbench.get_measurable(ref_line_2d_btm).length, 6)
    y = round(spa_workbench.get_measurable(ref_line_2d_left).length, 6)
    z = round(spa_workbench.get_measurable(ref_line_guide).length, 6)
